...
`CommandQueue` expects increasing positive sequence numbers over an ordered connection. Duplicate/older commands are rejected. It consumes at most one queued command per tick; a command beyond its capacity is an error, so a room can disconnect a client that has run far ahead instead of losing input silently. `applied` is the last consumed sequence, not a promise that every lower sequence ran. When configured to repeat, it repeats the last command for a bounded number of ticks, then calls the game's idle policy. **Use repetition only for held intent, not one-shot actions** such as buying, firing or digging. A game with those actions must disable repetition or use a separate action policy.
...
`Prediction` replays unacknowledged commands over an authoritative correction using a game-supplied deterministic function. It is not rollback for an entire interacting physics world. `SnapshotBuffer` interpolates remote states, extrapolates a bounded number of ticks past the latest state and then holds it. `RenderClock` runs the presentation tick a fixed delay behind the newest snapshot, eases toward it as latency changes and snaps when far off. The workshop demonstrates correction smoothing and a small adaptive render delay; it has not been tuned against a real-world latency/loss matrix.
...
`Recorder` stores commands and post-tick hashes with build and scene identities and an optional setup payload, which `verifyReplay` hands back to the simulation factory. `verifyReplay` constructs a fresh simulation, reports the first divergent zero-based tick and always disposes it. Games own complete command serialization and initial-state identity. This is replay-from-start verification, not a save-game format or arbitrary snapshot restoration API.
