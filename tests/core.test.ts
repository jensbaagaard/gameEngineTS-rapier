...
import { CommandQueue, Entities, FixedClock, Hasher, Prediction, Recorder, Replica, Replicator, Rng, Session, Simulation, SnapshotBuffer, TickInbox, canonical, verifyReplay } from '../src/index.js';
...
describe('fixed simulation', () => {
  ...
  it('orders phases and disposes exactly once', () => {
    const order: string[] = [];
    const release = vi.fn();
    const sim = new Simulation([{ name: 'movement', run: () => order.push('move') }, { name: 'physics', run: () => order.push('physics') }], release);
    sim.step(undefined);
    expect(order).toEqual(['move', 'physics']);
    sim.dispose(); sim.dispose();
    expect(release).toHaveBeenCalledTimes(1);
    expect(() => sim.step(undefined)).toThrow('disposed');
    expect(() => new Simulation([{ name: 'same', run() {} }, { name: 'same', run() {} }])).toThrow();
  });
  it('rejects recursive stepping and disposal inside a tick', () => {
    const sim = new Simulation([{ name: 'step', run: () => { expect(() => sim.step(undefined)).toThrow('reentrant'); expect(() => sim.dispose()).toThrow('during'); } }]);
    sim.step(undefined);
    sim.dispose();
  });
  it('uses stable ids and skips a removed or replaced object in the current tick', () => {
    ...
    const updated = vi.fn(), disposed = vi.fn();
    world.add({ update: updated, dispose: disposed }, 'b');
    world.add({ update: () => { world.remove('b'); world.add({ update: updated }, 'b'); } }, 'a');
    world.update();
  ...
  it('releases other entities even if one fails', () => {
    const entities = new Entities(); const release = vi.fn();
    entities.add({ dispose: () => { throw new Error('failed'); } });
  ...
  it('keeps the old scene on load failure and carries session state on success', () => {
    ...
    const session = new Session({ score: 12 }, (name, state) => {
      if (name === 'broken') throw new Error('load failed');
      return { name, score: state.score, dispose: () => { released.push(name); } };
    }, 'first');
    expect(() => session.change('broken')).toThrow();
    expect(session.level.name).toBe('first'); expect(session.epoch).toBe(0);
    session.change('second');
    expect(session.level.score).toBe(12); expect(session.epoch).toBe(1);
...
describe('network state', () => {
  it('applies commands once, rejects old sequences, repeats briefly and bounds backlog', () => {
    ...
    queue.push(3, { x: 3 }); queue.push(2, { x: 2 });
  ...
  it('projects public state before diffing and sends only changes, including removals', () => {
    ...
    const writer = new Replicator(), reader = new Replica();
    ...
    expect(unchanged.set).toEqual({}); reader.apply(unchanged);
    authority.field[0] = 1;
    const changed = writer.encode(project());
    expect(Object.keys(changed.set)).toEqual(['field']); reader.apply(changed);
    reader.apply(writer.encode({ field: authority.field }));
    expect(reader.state).toEqual({ field: [1, 1, 0] });
    writer.reset(); reader.apply(writer.encode({ nextScene: true }));
    expect(reader.state).toEqual({ nextScene: true });
  });
  it('detects dropped deltas and tolerates a duplicate', () => {
    const writer = new Replicator(), reader = new Replica();
    const first = writer.encode({ a: 1 }); reader.apply(first);
  ...
  it('never writes prototype properties through a patch', () => {
    ...
    reader.apply({ base: null, revision: 1, set: JSON.parse('{"__proto__":{"polluted":true}}'), remove: [] });
  ...
  it('replays only unacknowledged commands after a correction', () => {
    const prediction = new Prediction({ x: 0 }, (s, c: number) => { s.x += c; }, 3);
    prediction.push(1); prediction.push(2);
    prediction.correct({ x: 0.5 }, 1);
    expect(prediction.state.x).toBe(2.5);
    prediction.correct({ x: 2 }, 2); expect(prediction.state.x).toBe(2);
  ...
  it('interpolates across jitter and holds the latest sample without unbounded extrapolation', () => {
    ...
    buffer.push(1, 10); buffer.push(4, 40);
    ...
    buffer.clear(); expect(buffer.sample(2)).toBeUndefined();
  });
});

describe('deterministic utilities', () => {
  it('rejects state that cannot survive a JSON round trip', () => {
    for (const value of [NaN, undefined, new Array(2), new Map(), new Date(), { x: undefined }]) expect(() => canonical(value as any)).toThrow();
  ...
  it('restores the random stream and rejects invalid ranges', () => {
    const rng = new Rng(7); const state = rng.state();
  ...
  it('verifies replays, reports first divergence, and frees simulations on failure', () => {
    ...
    const create = () => {
      let n = 0;
      return { step: (c: number) => { n += c; }, hash: () => new Hasher().int(n).digest(), dispose };
    };
    const recorder = new Recorder<number>('v1', 'scene'); const sim = create();
    for (const c of [1, 2, 3]) { sim.step(c); recorder.record(c, sim.hash()); }
