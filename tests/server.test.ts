...
beforeEach(async () => {
  server = await startServer(0, '127.0.0.1', 80);
});
afterEach(async () => {
...
const join = (room: string, create = true, password = '') => ({
  type: 'join',
  version: VERSION,
  content: CONTENT,
  room,
...
describe('dedicated server', () => {
  it('refuses mismatched scene content and reports occupied ports', async () => {
    ...
    expect(await waitFor(() => a.messages.find((m) => m.type === 'error'))).toMatchObject({
      message: 'Scene content mismatch; reload',
    });
  ...
  it('validates unknown, null and malformed messages without disconnecting the player', async () => {
    ...
    await waitFor(() =>
      a.messages.filter((m) => m.type === 'error').length === 4 ? true : undefined,
    );
  ...
  it('acknowledges commands, changes scene without losing slots, and ignores stale inputs', async () => {
    ...
    const snapshot = await waitFor(() =>
      a.messages.find((m): m is Snapshot => m.type === 'snapshot' && m.epoch === 1),
    );
    expect(snapshot.scene).toBe('courtyard');
    ...
    await waitFor(() =>
      a.messages.find((m) => m.type === 'snapshot' && m.epoch === 1 && m.tick > snapshot.tick + 3),
    );
  ...
  it('bounds pre-join message floods and expires unused rooms', async () => {
    ...
    const closed = new Promise<number>((resolve) =>
      flood.socket.once('close', (code) => resolve(code)),
    );
    for (let i = 0; i < 151; i++) flood.send(null);
    expect(await closed).toBe(1008);
    const a = await connect();
    a.send(join('temporary'));
    await waitFor(() => a.messages.find((m) => m.type === 'welcome'));
    a.socket.close();
