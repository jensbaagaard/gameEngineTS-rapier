...
describe('scene documents', () => {
  ...
  it('does not depend on type registration order or contaminate other expansions', () => {
    const reversed = new SceneRegistry(
      Object.fromEntries([...registry.objects].reverse()),
      Object.fromEntries([...registry.generators].reverse()),
      registry.settings,
    );
  ...
  it.each([
    ...
    (s: any) => {
      s.objects.push(s.objects[0]);
    },
    (s: any) => {
      s.objects[0].type = 'unknown';
    },
    (s: any) => {
      s.objects[0].settings.size.x = -1;
    },
    (s: any) => {
      s.objects[0].settings.position.x = NaN;
    },
    (s: any) => {
      s.objects[0].settings.extra = 1;
    },
    (s: any) => {
      s.objects[2].seed = 0.5;
    },
