...
const vite = await createServer({ server: { middlewareMode: true, hmr: { server: server.http } }, appType: 'spa' }).catch(async error => { await server.close(); throw error; });
server.http.on('request', vite.middlewares);
const browser = await chromium.launch({ args: ['--use-angle=swiftshader', '--enable-unsafe-swiftshader'] }).catch(async error => { await vite.close(); await server.close(); throw error; });
const errors: string[] = [];
...
async function ticks(page: Page, count: number): Promise<void> {
  const until = await page.evaluate(count => (window as any).__engine.local.level.tick + count, count);
...
const x = (page: Page) => page.evaluate(() => (window as any).__engine.state['player:0'].position.x as number);
const memory = (page: Page) => page.evaluate(() => {
...
try {
  ...
  assert.deepEqual(browserHashes, nodeHashes, 'Node and Chromium must agree, including full Rapier snapshots');
  ...
  assert.match(await preview.locator('#status').innerText(), /Scene preview · no simulation running/);
  assert.equal(await preview.locator('#room').isVisible(), false);
  assert.equal(requests.some(url => /rapier|\/src\/physics\.ts|\/game\/simulation\.ts/.test(url)), false, 'Scene preview must not load physics');
  ...
  assert.equal(baked.objects.some(entry => 'seed' in entry), false);
  ...
  assert.equal(await b.locator('[name="room"]').inputValue(), 'browser', 'Text fields must retain typed input');
  ...
  assert.deepEqual([...server.rooms.get('browser')!.session.level.players.keys()], ['player:0', 'player:1']);
  ...
  process.stdout.write(`Browser checks passed: local input, disposal, preview, baking, two-player rooms, travel.\nNode/Chromium physics hashes: ${nodeHashes.map(hash => hash.toString(16)).join(', ')}\nCaptures: evidence/browser/\n`);
} finally {
  await browser.close();
  await vite.close();
  await server.close();
