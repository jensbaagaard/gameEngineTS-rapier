import { SceneRegistry, integer, meters, number, object, string, type SceneDocument, type Schema } from '../src/index.js';
...
export const registry = new SceneRegistry({
  ...
  boxes: {
    settings: object({ count: { ...integer, min: 0, max: 50 }, spread: { ...number, min: 1, max: 10 } }),
    generate(settings, random) {
      return Array.from({ length: settings.count as number }, (_, i) => ({
        id: String(i), type: 'box', settings: {
          position: { x: (random.range(-100, 100) / 100) * (settings.spread as number), y: 3 + i * 0.7, z: (random.range(-100, 100) / 100) * (settings.spread as number) },
          size: { x: 0.7, y: 0.7, z: 0.7 }, color: '#f2ad55',
        },
      }));
    },
  },
}, object({ gravity: object({ x: acceleration, y: acceleration, z: acceleration }), background: string }));
