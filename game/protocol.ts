...
const content = Object.fromEntries(
  Object.entries(scenes).map(([name, scene]) => [
    name,
    {
      ...
      objects: registry.expand(scene).map((entry) => ({ ...entry })),
    },
  ]),
);
...
const schemas: Record<string, Schema> = {
  ...
  command: object({
    type: { type: 'string', values: ['command'] },
    epoch: nonnegative,
    sequence: { ...integer, min: 1 },
    command: commandSchema,
  }),
  scene: object({
    type: { type: 'string', values: ['scene'] },
...
export function parseClientMessage(data: string): ClientMessage {
  ...
  if (
    ...
    !Object.hasOwn(schemas, message.type)
  )
