...
export function validate(schema: Schema, value: unknown, path = '$'): void {
  ...
  if (schema.type === 'object') {
    ...
    for (const key of Object.keys(data))
      if (!Object.hasOwn(schema.properties, key))
        throw new Error(`${path}.${key}: unknown setting`);
    for (const [key, field] of Object.entries(schema.properties))
      validate(field, Object.hasOwn(data, key) ? data[key] : undefined, `${path}.${key}`);
    return;
  }
  if (schema.type === 'array') {
    if (
      ...
      value.length > (schema.max ?? Infinity)
    )
  ...
  if (schema.type === 'null' ? value !== null : typeof value !== schema.type)
    throw new Error(`${path}: expected ${schema.type}`);
  if (
    schema.type === 'number' &&
    (!Number.isFinite(value) ||
      (value as number) < (schema.min ?? -Infinity) ||
      (value as number) > (schema.max ?? Infinity) ||
      (schema.integer && !Number.isSafeInteger(value)))
  )
    throw new Error(`${path}: invalid number`);
  if (
    schema.type === 'string' &&
    ((schema.values && !schema.values.includes(value as string)) ||
      (value as string).length > (schema.maxLength ?? Infinity))
  )
