...
export interface Generator extends SceneObject {
  seed: number;
}
...
export class SceneRegistry {
  ...
  parse(value: unknown): SceneDocument {
    ...
    if (
      ...
      !raw.id ||
      !properties(raw.settings) ||
      !Array.isArray(raw.objects)
    )
      throw new Error('Invalid scene document');
    for (const key of Object.keys(raw))
      if (!['version', 'id', 'settings', 'objects'].includes(key))
        throw new Error(`Unknown scene property: ${key}`);
    ...
    for (const entry of scene.objects) {
      ...
      const entries =
        'seed' in entry
          ? this.generators
              .get(entry.type)!
              .generate(entry.settings, new Rng(entry.seed))
              .map((child) => {
