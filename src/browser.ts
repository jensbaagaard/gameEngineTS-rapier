...
export class PointerInput {
  ...
  constructor(readonly canvas: HTMLCanvasElement) {
    ...
    window.addEventListener(
      'mousemove',
      (event) => {
        if (this.locked || event.target === canvas) {
  ...
  releaseLock(): void {
    if (this.locked) this.canvas.ownerDocument.exitPointerLock();
    this.clear();
  }
...
export class RenderObjects {
  ...
  dispose(): void {
    const resources = new Set(
      [...this.objects.values()].flatMap((object) => [...resourcesOf(object)]),
    );
