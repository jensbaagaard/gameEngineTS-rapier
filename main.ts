import { FixedClock, Prediction, Replica, Session, SnapshotBuffer, TickInbox, type Properties } from './src/index.js';
...
let previous: Properties = {}, current: Properties = {};
let last: number | undefined;
let frame = 0;
let closed = false;
view.load('workshop');
roomForm.hidden = mode !== 'online';
status.textContent = mode === 'preview' ? 'Scene preview · no simulation running' : mode === 'online' ? 'Create or join a room' : 'Local game · WASD to move';

if (mode === 'local') {
  const { initPhysics } = await import('./src/physics.js');
  const { DemoSimulation } = await import('./game/simulation.js');
  await initPhysics();
  local = new Session({ changes: 0 }, scene => {
...
function command(): Command {
  return { x: +keyboard.down('KeyD') - +keyboard.down('KeyA'), z: +keyboard.down('KeyS') - +keyboard.down('KeyW') };
}
...
roomForm.addEventListener('submit', event => {
  ...
  ws.onopen = () => ws.send(JSON.stringify({ type: 'join', version: VERSION, content: CONTENT, room: form.get('room'), password: form.get('password'), create }));
  ws.onmessage = event => {
    if (socket !== ws) return;
    try {
      const message = JSON.parse(String(event.data)) as ServerMessage;
      if (message.type === 'error') { status.textContent = message.message; if (!prediction) disconnect(message.message); }
      else if (message.type === 'welcome') {
        if (message.version !== VERSION) throw new Error('Version mismatch; reload');
        playerId = message.id;
        epoch = message.epoch;
        inbox.clear(); history.clear(); replica = new Replica(); renderTick = -1;
    ...
    } catch (error) { disconnect(error instanceof Error ? error.message : 'Connection failed'); }
  };
  ws.onclose = event => { if (socket === ws) disconnect(event.reason || 'Disconnected · join again to synchronize'); };
  ws.onerror = () => { if (socket === ws) disconnect('Could not connect'); };
}, { signal: listeners.signal });

for (const button of document.querySelectorAll<HTMLButtonElement>('[data-scene]')) button.addEventListener('click', () => {
  const scene = button.dataset.scene!;
  if (mode === 'online') {
    if (socket?.readyState === WebSocket.OPEN && prediction) socket.send(JSON.stringify({ type: 'scene', scene }));
    return;
  }
  if (local) { local.change(scene); local.state.changes++; current = local.level.state(); previous = current; clock.reset(); }
  view.load(scene);
  status.textContent = mode === 'preview' ? `${scene} · no simulation running` : `${scene} · ${local!.state.changes} scene changes · WASD to move`;
}, { signal: listeners.signal });

document.querySelector('#export')!.addEventListener('click', () => {
  const document = registry.bake(getScene(view.currentScene), 'stack');
  const url = URL.createObjectURL(new Blob([registry.serialize(document)], { type: 'application/json' }));
  const anchor = window.document.createElement('a');
  anchor.href = url; anchor.download = `${document.id}.scene.json`; anchor.click();
...
function receive(): void {
  for (const snapshot of inbox.drain()) {
    ...
    if (prediction && me) {
      ...
      const dx = before.x - prediction.state.x, dz = before.z - prediction.state.z;
      if (!changed && Math.hypot(dx, dz) < 3) { correction.x += dx; correction.z += dz; }
      else correction = { x: 0, y: 0, z: 0 };
    }
    history.push(snapshot.tick, replica.state);
    for (const event of snapshot.events) status.textContent = `${event.scene} · ${snapshot.changes} scene changes · ${playerId}`;
  }
}
...
function draw(now: number): void {
  ...
  try {
    ...
    const alpha = clock.advance(elapsed, () => {
      const input = command();
      if (local) { previous = current; local.level.step(new Map([[playerId, input]])); current = local.level.state(); }
    ...
    if (local) view.draw(previous, current, alpha, playerId);
      ...
      else { renderTick += elapsed / 1000 * TPS; renderTick += (target - renderTick) * (1 - Math.exp(-elapsed / 500)); }
      const sample = history.sample(renderTick)!;
      correction.x *= Math.exp(-elapsed / 80); correction.z *= Math.exp(-elapsed / 80);
      const position = prediction ? { x: prediction.state.x + correction.x, y: prediction.state.y, z: prediction.state.z + correction.z } : undefined;
      view.draw(sample.from, sample.to, sample.alpha, playerId, position);
    } else view.render();
  } catch (error) { disconnect(error instanceof Error ? error.message : 'Simulation failed'); }
  if (!closed) frame = requestAnimationFrame(draw);
}
frame = requestAnimationFrame(draw);
window.addEventListener('pagehide', () => {
  closed = true; cancelAnimationFrame(frame); socket?.close(); local?.dispose(); keyboard.dispose(); listeners.abort(); view.dispose();
}, { once: true });

Object.assign(window, { __engine: { mode, view, get local() { return local; }, get state() { return local?.level.state() ?? replica.state; } } });
